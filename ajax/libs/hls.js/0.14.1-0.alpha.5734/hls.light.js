...
var mp4_remuxer_MP4Remuxer = /*#__PURE__*/function () {
  ...
  _proto.remuxVideo = function remuxVideo(track, timeOffset, contiguous, audioTrackLength, accurateTimeOffset) {
    ...
    var firstDTS;
    var lastDTS;
    var minPTS = Number.MAX_SAFE_INTEGER;
    var maxPTS = -Number.MAX_SAFE_INTEGER;
    ...
    lastDTS = inputSamples[inputSamples.length - 1].dts; // check timestamp continuity across consecutive fragments (this is to remove inter-fragment gap/hole)

    var delta = firstDTS - nextAvcDts; // if fragment are contiguous, detect hole/overlapping between fragments

    if (contiguous) {
      var foundHole = delta > 2;
      ...
      if (foundHole || foundOverlap) {
        ...
        logger["logger"].log("Video: PTS/DTS adjusted: " + toMsFromMpegTsClock(minPTS, true) + "/" + toMsFromMpegTsClock(firstDTS, true) + ", delta: " + toMsFromMpegTsClock(delta, true) + " ms");
      }
    ...
    for (var _i = 0; _i < nbSamples; _i++) {
      ...
      if (isSafari) {
        ...
        sample.dts = firstDTS + _i * mp4SampleDuration;
      } else {
