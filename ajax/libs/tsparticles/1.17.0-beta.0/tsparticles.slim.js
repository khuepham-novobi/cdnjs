...
class Particle_Particle {
  constructor(container, position, overrideOptions) {
    ...
    this.infecter = new Infecter_Infecter(this.container, this);
  }
...
class Mouse_Attractor_Attractor {
  ...
  clickAttract() {
    ...
    if (container.attract.clicking) {
      ...
      const range = new Circle_Circle(mouseClickPos.x, mouseClickPos.y, attractRadius);
      const query = container.particles.quadTree.query(range);
  ...
  processClickAttract(particle, dx, dy, force) {
    const container = this.container;
    const options = container.options;
    const f = Math.atan2(dy, dx);
    particle.velocity.horizontal = -force * Math.cos(f);
...
class main_slim_MainSlim {
  ...
  set(id, element, params) {
    return __awaiter(this, void 0, void 0, function* () {
      return Loader_Loader.set(id, element, params);
    });
  }
