...
__webpack_require__.d(__webpack_exports__, "CircleWarp", function() { return /* reexport */ CircleWarp_CircleWarp; });
...
__webpack_require__.d(__webpack_exports__, "Point", function() { return /* reexport */ Point; });
...
class Utils_Utils {
  ...
  static getDistance(pointA, pointB) {
    return Utils_Utils.getDistances(pointA, pointB).distance;
  }
  ...
  static itemFromArray(array, index) {
    return array[index !== null && index !== void 0 ? index : Utils_Utils.arrayRandomIndex(array)];
  }
  ...
  static isPointInside(point, size, radius) {
    return Utils_Utils.areBoundsInside(Utils_Utils.calculateBounds(point, radius !== null && radius !== void 0 ? radius : 0), size);
  }
  ...
  static deepExtend(destination, ...sources) {
    for (const source of sources.filter(s => s !== undefined && s !== null)) {
      ...
      for (const key in source) {
        ...
        destination[key] = isObject && Array.isArray(value) ? value.map(v => Utils_Utils.deepExtend(destination[key], v)) : Utils_Utils.deepExtend(destination[key], value);
      }
    }
...
class Canvas_Canvas {
  ...
  drawParticleShadow(particle, mousePos) {
    ...
    CanvasUtils_CanvasUtils.drawParticleShadow(this.container, this.context, particle, radius, mousePos);
  }
  ...
  updateLife(delta) {
    ...
    if (!particle.spawning) {
      ...
      if (particle.lifeTime >= particle.lifeDuration) {
        ...
        particle.position.x = Utils_Utils.randomInRange(0, canvasSize.width);
        particle.position.y = Utils_Utils.randomInRange(0, canvasSize.height);
        ...
        if (lifeOptions.delay.random.enable) {
          particle.lifeDelay = Utils_Utils.randomInRange(lifeOptions.delay.random.minimumValue, lifeOptions.delay.value) * 1000;
        }

        if (lifeOptions.duration.random.enable) {
          particle.lifeDuration = Utils_Utils.randomInRange(lifeOptions.duration.random.minimumValue, lifeOptions.duration.value) * 1000;
        }
      }
    }
  }
  ...
  updateBounce(delta) {
    ...
    if (outMode === OutMode.bounce || outMode === OutMode.bounceHorizontal) {
      ...
      if (bounds.right >= canvasSize.width && velocity > 0 || bounds.left <= 0 && velocity < 0) {
        ...
        const newVelocity = factor.random.enable ? Utils_Utils.randomInRange(factor.value, factor.random.minimumValue) : factor.value;
...
// CONCATENATED MODULE: ./dist/Options/Classes/Particles/Bounce/BounceFactorRandom.js
class BounceFactorRandom {
  constructor() {
    this.enable = false;
...
class BounceFactor_BounceFactor {
  constructor() {
...
class Particle_Particle {
  constructor(container, position, overrideOptions) {
      ...
      if (!randomSize) {
        switch (sizeAnimation.startValue) {
          ...
          case StartValueType.random:
            this.size.value = Utils_Utils.randomInRange(sizeAnimation.minimumValue * pxRatio, this.size.value);
            this.size.status = SizeAnimationStatus.increasing;
            break;
    ...
    if (lifeOptions.delay.random.enable) {
      lifeDelay = Utils_Utils.randomInRange(lifeOptions.delay.random.minimumValue, lifeDelay);
    ...
    if (lifeOptions.duration.random.enable) {
      lifeDuration = Utils_Utils.randomInRange(lifeOptions.duration.random.minimumValue, lifeDuration);
...
class Options_Options {
  ...
  setTheme(name) {
    if (name) {
      ...
      let defaultTheme = defaultThemes.find(theme => theme.default.mode === ThemeMode.dark && clientDarkMode || theme.default.mode === ThemeMode.light && !clientDarkMode);
