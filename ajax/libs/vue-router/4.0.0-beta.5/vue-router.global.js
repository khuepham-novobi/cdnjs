...
var VueRouter = (function (exports, vue) {
  ...
  function onBeforeRouteLeave(leaveGuard) {
      ...
      if (!instance) {
              ...
              vue.warn('onBeforeRouteLeave must be called at the top of a setup function');
          return;
      }
  ...
  function onBeforeRouteUpdate(updateGuard) {
      ...
      if (!instance) {
              ...
              vue.warn('onBeforeRouteUpdate must be called at the top of a setup function');
          return;
      }
      const activeRecord = vue.inject(matchedRouteKey, {}).value;
      if (!activeRecord) {
      ...
      return () => new Promise((resolve, reject) => {
          ...
          if ( guard.length > 2)
              guardCall = guardCall.then(() => {
                  ...
                  if (!next._called)
                      vue.warn(`The "next" callback was never called inside of ${guard.name ? '"' + guard.name + '"' : ''}:\n${guard.toString()}\n. If you are returning a value instead of calling "next", make sure to remove the "next" parameter from your function.`);
  ...
  function canOnlyBeCalledOnce(next, to, from) {
      ...
      return function () {
          if (called++ === 1)
              vue.warn(`The "next" callback was called more than once in one navigation guard when going from "${from.fullPath}" to "${to.fullPath}". It should be called exactly one time in each navigation guard. This will fail in production.`);
      ...
      for (const record of matched) {
          for (const name in record.components) {
              ...
              if ( 'then' in rawComponent) {
                  vue.warn(`Component "${name}" in record with path "${record.path}" is a Promise instead of a function that returns a Promise. Did you write "import('./MyPage.vue')" instead of "() => import('./MyPage.vue')"? This will break in production if not fixed.`);
              ...
              if (isRouteComponent(rawComponent)) {
                  ...
                  if ( !('catch' in componentPromise)) {
                      vue.warn(`Component "${name}" in record with path "${record.path}" is a function that does not return a Promise. If you were passing a functional component, make sure to add a "displayName" to the component. This will break in production if not fixed.`);
