...
(function (global, factory) {
  ...
  class WKTParser {
    ...
    read(wkt) {
      ...
      if (matches) {
        ...
        str = matches[2];
        if (parse[type]) geometry = parse[type].call(this, str);
      }
    ...
    extractGeometry(geometry) {
      ...
      const wktType = type.toUpperCase();
      let data;
      if (geometry.isEmpty()) data = wktType + ' EMPTY';else data = wktType + '(' + extract[type].call(this, geometry) + ')';
      return data;
    }
  ...
  const extract = {
    ...
    multilinestring(multilinestring) {
      ...
      for (let i = 0, len = multilinestring._geometries.length; i < len; ++i) array.push('(' + extract.linestring.call(this, multilinestring._geometries[i]) + ')');
  ...
  const parse = {
    ...
    linestring(str) {
      ...
      for (let i = 0, len = points.length; i < len; ++i) components.push(parse.coord.call(this, points[i]));
    ...
    multilinestring(str) {
      ...
      let line;
      const lines = str.trim().split(regExes.parenComma);
      const components = [];
    ...
    polygon(str) {
      ...
      const rings = str.trim().split(regExes.parenComma);
      let shell;
      const holes = [];
    ...
    multipolygon(str) {
      ...
      let polygon;
      const polygons = str.trim().split(regExes.doubleParenComma);
      const components = [];
    ...
    geometrycollection(str) {
      if (str === undefined) return this.geometryFactory.createGeometryCollection(); // separate components of the collection with |

      str = str.replace(/,\s*([A-Za-z])/g, '|$1');
      const wktArray = str.trim().split('|');
      const components = [];

      for (let i = 0, len = wktArray.length; i < len; ++i) components.push(this.read(wktArray[i]));
