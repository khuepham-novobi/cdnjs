/*!
  ...
  */
import { ref, getCurrentInstance, computed, watch, createVNode, Text, h, Fragment, inject, onMounted, onUnmounted } from 'vue';
...
function createComposer(options = {}) {
    ...
    function __transrateVNode(...args) {
        return wrapWithDeps(context => {
            ...
            return ret;
        }, () => parseTranslateArgs(...args)[0], 'translate', root => root.__transrateVNode(...args), key => [createVNode(Text, null, key, 0)], val => isArray(val));
    }
...
function createI18n(options = {}) {
    ...
    const symbol = Symbol((process.env.NODE_ENV !== 'production') ? 'vue-i18n' : '');
    const i18n = {
        get mode() {
...
function setupLifeCycle(i18n, target, composer) {
    onMounted(() => {
        if (((process.env.NODE_ENV !== 'production') || __INTLIFY_PROD_DEVTOOLS__) && target.vnode.el) {
